...
//        Initiate widgets
        txtDailyTotal = view.findViewById(R.id.txt_report_total);
...
//    load daily-total-sales
    public void onLoadDailyTotal(String url) {
        StringRequest request = new StringRequest(Request.Method.POST, MyUrl.setUrl(url), new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                String defualtValue = getResources().getString(R.string.txt_report_daily_total);
                txtDailyTotal.setText(defualtValue.concat(" $" + response));
            }
        }, new Response.ErrorListener() {
            @Override
            public void onErrorResponse(VolleyError error) {
