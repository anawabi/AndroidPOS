...
import com.xamuor.cashco.Model.SettingsDataModal;
import com.xamuor.cashco.UserRoleBottomSheet;
...
public class SettingsAdapter extends RecyclerView.Adapter<SettingsAdapter.ViewHoler> {
    ...
    public void onBindViewHolder(final ViewHoler holder, int position) {
        ...
        Glide.with(context).load(Routes.onLoadImage("user_photos/").concat(modal.getPhoto())).into(holder.userPhoto);
...
//            txtPhone = itemView.findViewById(R.id.txt_user_phone);
    ...
    private void onChangeUserStatus(final int userId, final int statusValue, final String user) {
        StringRequest statusRequest = new StringRequest(Request.Method.POST, Routes.setUrl("userStatus"), new Response.Listener<String>() {
            @Override
